...
const ireneTaskSchema = Joi.object({
  ...
  notes: Joi.string().allow('').max(1000).optional(),
  category: Joi.alternatives().try(Joi.string().allow('', 'Default').max(100), Joi.valid(null)).optional(),
  difficulty: Joi.number().integer().min(1).max(10).optional(),
}).unknown(true);
...
app.get('/api/tasks', authMiddleware, async (req, res) => {
  // Per requirements, load all tasks where the partition key user_id equals the user's email
  const uemail = String(req.user.email);

  let rows = [];
  try {
    rows = await ddbListTasks(uemail);
  } catch (e) {
    console.warn('[tasks] DDB list failed for email', { uemail, err: String(e?.message || e) });
    return res.status(500).json({ error: 'Failed to list tasks. Please try again later.' });
  }
  ...
  res.setHeader('X-TTT-Tasks-Src', `email:${rows.length}`);
  res.json({ tasks });
});
...
app.get('/api/irene/tasks', authMiddleware, async (req, res) => {
  try {
    ...
    const tasks = rows.map(r => ({ id: r.id, title: r.title, notes: r.notes || '', category: r.category || 'Default', difficulty: Number.isFinite(r.difficulty) ? r.difficulty : (r.difficulty != null ? Number(r.difficulty) : undefined) }));
    res.json({ tasks });
...
app.post('/api/irene/tasks', authMiddleware, async (req, res) => {
  try {
    ...
    const item = { user_id: String(gid), id, title: String(value.title), notes: (value.notes != null ? String(value.notes) : ''), category: (value.category == null || value.category === '' ? 'Default' : String(value.category)), difficulty: (Number.isFinite(value.difficulty) ? value.difficulty : (value.difficulty != null ? Number(value.difficulty) : undefined)) };
...
app.get('/api/irene/analytics', authMiddleware, async (req, res) => {
  try {
    ...
    const taskDifficulty = {}; for (const t of tasksList) { const d = Number.isFinite(t.difficulty) ? t.difficulty : Number(t.difficulty || 1); taskDifficulty[String(t.id)] = (d>=1 && d<=10) ? d : 1; }
    ...
    const scoreByUser = {}; // email -> weighted score by difficulty
    ...
    for (const l of logs) {
      ...
      const k = day + '|' + tid;
      const weight = taskDifficulty[tid] || 1;
      byDayTaskAll[k] = (byDayTaskAll[k] || 0) + 1;
      byUser[email] = (byUser[email] || 0) + 1;
      scoreByUser[email] = (scoreByUser[email] || 0) + weight;
