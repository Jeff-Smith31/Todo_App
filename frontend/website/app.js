...
(function(){
  ...
  async function init(){
    ...
    // Auto-detect backend base if same-origin /api is not routed. This helps when CloudFront/Nginx
    ...
    async function detectBackendBaseIfNeeded(){
      ...
      try {
        ...
        // If response looks like JSON (whether 200 or 401) assume same-origin API works
  ...
  function ensureCategoryState(){
    // Individual categories (migrate from legacy settings.categories)
    if (!Array.isArray(settings.categoriesIndividual)) {
      const legacy = Array.isArray(settings.categories) ? settings.categories : null;
      settings.categoriesIndividual = legacy && legacy.length ? legacy.slice() : ['Default'];
      delete settings.categories; // migrate once
    }
    if (!settings.catOpenIndividual || typeof settings.catOpenIndividual !== 'object') settings.catOpenIndividual = {};
    if (typeof settings.catOpenIndividual['Default'] === 'undefined') settings.catOpenIndividual['Default'] = true;

    // Family (Irene) categories
    if (!Array.isArray(settings.categoriesIrene)) settings.categoriesIrene = ['Default'];
    if (!settings.catOpenIrene || typeof settings.catOpenIrene !== 'object') settings.catOpenIrene = {};
    if (typeof settings.catOpenIrene['Default'] === 'undefined') settings.catOpenIrene['Default'] = true;

    // Migrate tasks without category (individual list)
    let changed = false;
